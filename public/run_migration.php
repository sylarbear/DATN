<?php
/**
 * Run migration v2 - Robust version
 * Handles semicolons inside string values correctly
 */
require_once __DIR__ . '/../app/config/database.php';
$db = getDB();

echo "<h3>Running Migration v2...</h3><pre>";

// Execute each statement individually
$statements = [
    // 1. ALTER users table for streak/XP
    ...
    "ALTER TABLE users ADD COLUMN daily_xp_today INT DEFAULT 0",
    "ALTER TABLE users ADD COLUMN daily_goal_date DATE DEFAULT NULL",
    ...
    // 3. Vocab Reviews (Spaced Repetition)
...
echo "\n--- Seeding Grammar Data ---\n";
$check = $db->query("SELECT COUNT(*) as c FROM grammar_lessons")->fetch();
if ($check['c'] == 0) {
    $lessons = [
    ...
    $stmt = $db->prepare("INSERT INTO grammar_lessons (title, slug, category, level, content_html, examples, sort_order) VALUES (?,?,?,?,?,?,?)");
    foreach ($lessons as $l) { $stmt->execute($l); }
    echo "Inserted " . count($lessons) . " grammar lessons\n";
    ...
    $stmt = $db->prepare("INSERT INTO grammar_questions (grammar_lesson_id, question_text, options, correct_answer, explanation) VALUES (?,?,?,?,?)");
    foreach ($questions as $q) { $stmt->execute($q); }
    echo "Inserted " . count($questions) . " grammar questions\n";
} else {
    echo "Grammar data already exists (" . $check['c'] . " lessons)\n";
}
